...
bicep_image=Blueprint('bicep_image',__name__)
...
def classifyPose(landmarks, output_image):
    ...
    shoulder1 = [landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value],
                 landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value],
                 landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value]]

    angle = calculate_angle(*shoulder)
    angle1 = calculate_angle(*shoulder1)

    if angle > 160:
        counter +=1
        if angle1 > 160 :
            label = 'Right and Left Bicep True'
        else:
            label = 'Left Bicep  is True'
...
@bicep_image.route('/bicepImage', methods=['POST'])
def detect_pose():
